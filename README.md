...
> `make phase3-bench` and written into [`EVALUATION.md`](EVALUATION.md) §6.2. This is the
> **definitive re-validation** (2026-07-19) with the improved, two-sided span-sufficiency
> verifier prompt (§6.5) — the paper's headline result. **Free-tier-bounded** (8B model,
> single seeded run); the earlier §6.4 cross-model study (8B/70B/550B) still reflects the
...
| --- | --- | --- | --- | --- | --- |
| Single-LLM baseline | 60.0% | 60.3% | 37.7% | 0.30 / 8.93 / 12.95 s | 1388.0 |
| Multi-agent, ungrounded (ablation) | 65.0% | 65.5% | 35.7% | 14.06 / 19.00 / 21.66 s | 1473.2 |
| **Aletheia** (grounded verifier) | **69.0%** | **82.8%** | **23.8%** | 0.41 / 0.57 / 1.11 s | 1675.6 |

The grounded verifier **catches significantly more hallucinations** than the single-LLM
baseline — **82.8% vs 60.3%** (paired Δ +22.4 pp, 95% CI [+12.1, +33.3], excludes zero) —
and the ablation orders as the thesis predicts: single-LLM < ungrounded multi-agent <
grounded. Unlike the earlier preliminary-verifier run, **aggregate accuracy improves too**
(69.0% vs 60.0%, +9.0 pp, though the McNemar test on this specific comparison is not itself
significant at n=100). An error analysis (§6.3) shows why: the improved verifier prompt
roughly halved false-grounding errors on claims the corpus can't settle (21 → 10 of 37),
while abstention on answerable claims rose only modestly — a favorable trade that lifts
both catch rate and accuracy together. False-agreement also improved significantly (23.8%
vs 37.7%, Δ -13.9 pp, 95% CI [-23.9, -5.0]). Full reading, significance, and error
breakdown in [`EVALUATION.md`](EVALUATION.md) §6.2–§6.3.
...
- ✅ **Phase 3 — The evaluation harness (centerpiece)**: the SciFact benchmark and
  ...
  *(Live numbers on a 100-claim SciFact sample — grounded catch rate 82.8% vs 60.3%
  baseline, +22.4 pp with a 95% CI excluding zero, from the definitive re-validation
  with the improved verifier; see the table above and
