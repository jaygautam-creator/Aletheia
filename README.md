...
> **Free-tier-bounded** (8B model, single seeded run). The catch-rate result is robust
> across model scale — 8B → 70B → 550B (§6.4) — and an in-progress verifier refinement
> lifts *held-out* grounded accuracy by +13 pp on a small sample (§6.5, preliminary);
> re-running the n=100 headline with it is the next step.
